...
var BaseForm = (function (_Component) {
  ...
  BaseForm.prototype.addField = function addField(field, size) {
    if (!_lodash2['default'].isEmpty(field)) {
      ...
      switch (field.type) {
        ...
        case 'dropdown':
          return _react2['default'].createElement(_types.GenDropDown, { submit: this.submitForm, formName: this.props.formName, formKey: this.props.formKey, dispatch: this.props.dispatch, key: field.name, field: field, size: size, properties: properties }); // inputType.input(field, size);
        case 'success':
        case 'error':
          return _react2['default'].createElement(_types.GenMessage, { key: field.type, displayErrors: this.state.displayErrors, field: field, size: size, properties: properties, valid: this.props.valid, invalid: this.props.invalid, pristine: this.props.pristine, getActionState: this.props.getActionState }); // return this.message(field, size);
        case 'file':
  ...
  BaseForm.prototype.submit = function submit(e, handleSubmit) {
    e.preventDefault();
    this.setState({ 'displayErrors': true }, function () {
      handleSubmit(e);
    });
  };
